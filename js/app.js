// ---------------------------------------------------------------------------
// Milestone 1 connection check (v9+ modular SDK).
//
// Verifies, in order:
...
//
// Revision note: this performs a READ-ONLY check — it queries for up to 1
// document in a "systemStatus" collection and simply confirms Firestore
// responds (empty results still count as success). It does not write any
// document, so no throwaway test data is ever created in the database.
// ---------------------------------------------------------------------------

import { firebaseConfig } from "./firebase-config.js?v=0.9.9";
import { auth, db, firebaseInitError } from "./firebase-init.js?v=0.9.9";
...
async function runChecks() {
  ...
  try {
    ...
    setOverall('pass', 'All checks passed. Firebase and Firestore are ready for Milestone 2.');
  } catch (err) {
