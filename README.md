...
# Milestone 17: Final UI/CSS Polish Pass

CSS-only pass across all 9 pages, performed after v0.9.7's QA patch.
No JavaScript logic, Firestore schema, RBAC, or layout changes — see
CHANGELOG.md's v0.9.8 entry for the itemized list of fixes.

**What this pass found:** one hardcoded value that should have used an
existing token (`.cal-month-entry`'s border-radius), and 8 rule blocks
across the stylesheet that were exact duplicates of another rule
elsewhere — most notably a 3-line focus-treatment block repeated
verbatim 6 times. All were consolidated into shared rules with combined
selector lists; every affected selector still resolves to the exact
same declarations as before, so nothing renders differently. Net
effect: `css/styles.css` shrank (8 fewer rule blocks) rather than grew.

**What this pass reviewed and found already consistent:** card
spacing/radius/shadows, table header/row height (structural, not
coincidental — every table shares one `.data-table` class with zero
page-specific overrides), button states, navigation, focus-visible
coverage, and responsive spacing for the two newest pages. Confirmed via
